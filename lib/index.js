...
module.exports = function (share) {
    ...
    /*
     * 1. Clone repository to a known directory if it doesn't exist.
     * 2. Pull repository.
     * 3. Checkout cloned repository to the specific revision.
     * 4. Export current files to (checkout-index) revision-specific directory.
     * 5. If a Jakefile or Jakefile.js is found in the project's root directory,
     * 6  execute it. Otherwise perform npm install.
     * 7. Adjust symbolic link to point to the new location.
    ...
    task('clean', [ 'launch:symlink'], function() {
        action.success('Removed outdated deployments.');
        complete();
    }, true);
    ...
    task('checkout', [ 'launch:clone' ], function() {
        ...
        cmd_checkout += " checkout " + share.rev;

        action.local(cmd_checkout, function(ret) {
            if (ret == 0 ) {
                action.success("Checkout of " + share.rev + " successful.");
                complete();
            } else {
